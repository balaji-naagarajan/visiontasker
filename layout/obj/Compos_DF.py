...
import sys
import os
sys.path.append(os.path.abspath('D:\\Work\\python-ws\\vision-tasker\\VisionTasker-main\\VisionTasker-main\\CLIP'))
import clip
import torch
import re
...
class ComposDF:
    def __init__(self, json_file=None, json_data=None, gui_img=None):
        ...
        self.ori_compos_dataframe = None  # merge之后原本的df
        self.img = gui_img
    ...
    def closer_group_by_mean_area(self, compo_index, group1, group2, alignment):
        ...
        if compo['class'] != 'Text':
            ...
            if len(g1) * 3 < len(g2):  # 数量悬殊大 直接比平均
                ...
                g2_std = np.append(g2['area'].values, compo_area).std()
                if g2_std == 0:
                    g2_std = 0.0001
                if len(g1) == len(g2) and 0.5 < np.append(g1['area'].values, compo_area).std()/g2_std < 3:
    ...
    ******************************
    ...
    def split_groups(self, group_name):
        compos = self.compos_dataframe
        compos = compos.sort_values('center_row')
        groups = []
    ...
    ******************************
    ...
    def line_split(self, col_lines, row_lines):
        ...
        df_sorted.insert(df_sorted.shape[1], 'line_split_r', 0)  # 新增row列 记录行分割出的结果
        df_sorted.insert(df_sorted.shape[1], 'line_split_c', '0_0')  # 新增col列 记录列分割的结果
        ...
        #
        # # 横向group_pair也不能跨线
        # for gp in df_sorted['group_pair'].unique():
        #     if gp != -1:
        #         gp_df = df_sorted.loc[df_sorted['group_pair'] == gp]  # 取这一项 group_pair
        #         if gp_df['alignment_in_group'].iloc[0] == 'h':
        #             line_split_c = gp_df['line_split_c']  # 看线框分组有哪些
        #             if len(line_split_c.unique()) > 1:  # 线框分组不在一组
        #                 st_gp_name, st_li_name = max(df_sorted['group_pair']) + 1, max(df_sorted['list_item']) + 1  # 拆分 从最大开始编号
        #                 for lc in line_split_c.unique():
        #                     if len(gp_df.loc[gp_df['line_split_c'] == lc]) == 1:  # 如果拆完剩一个
        #                         df_sorted.loc[gp_df.loc[gp_df['line_split_c'] == lc].index, ['group_pair', 'pair_to', 'list_item']] = -1, -1, -1  # 清空分组
        #                     else:  # 如果不止一个
        #                         df_sorted.loc[gp_df.loc[gp_df['line_split_c'] == lc].index, ['group_pair', 'list_item']] = st_gp_name, st_li_name  # 更新编号
        #                         st_gp_name += 1  # 如果分成多组 还要+1
        #                         st_li_name += 1
        #                         for l_idx in gp_df.loc[gp_df['line_split_c'] == lc].index:
        #                             if gp_df.loc[l_idx, 'pair_to'] not in gp_df.loc[gp_df['line_split_c'] == lc].index:  # 如果配对的不在了
        #                                 df_sorted.loc[l_idx, 'pair_to'] = list(set(gp_df.loc[gp_df['line_split_c'] == lc].index) - {l_idx})[0]  # pair_to 随便选一个组内的
        #
        #
    ...
    '''
    **********************************
    * Recover the Seach Icon from Q *
    **********************************
    '''
    def q_search_recover(self, lang='zh'):
        ...
        for t_idx in self.compos_dataframe.index:
            if self.compos_dataframe.loc[t_idx, 'center_row'] < self.img.shape[1]/3 and \
                    re.search(pattern, self.compos_dataframe.loc[t_idx, 'text_content']) and 'QQ' not in self.compos_dataframe.loc[t_idx, 'text_content']:  # 判断疑似Q开头的搜索框 (前提搜索框位于图片上1/3)
                Q_idx.append(t_idx)

        self.compos_dataframe['text_content'] = self.compos_dataframe['text_content'].str.replace('\n', '')  # 去掉换行
        ...
        for q_idx in Q_idx:
            ...
            Q_center, Q_text_content, Q_sub_class = (Q_ccol, int(self.compos_dataframe.loc[q_idx, 'center_row'])), '搜索、放大_' if lang=='zh' else 'Search, zoom in', 'buttonicon'
            # 分裂文本
            ...
            self.compos_dataframe.loc[tx_idx, ['column_min', 'width', 'area', 'center_column', 'text_content', 'sub_class']] = \
    ...
    *****************************
    '''
    def check_ratio_anno(self, lang='zh'):
        cr_idx = self.compos_dataframe.loc[(self.compos_dataframe['sub_class'].str.lower().isin(['多选框' if lang=='zh' else 'check box', '选项' if lang=='zh' else 'options'])) &
                                           (self.compos_dataframe['width']/self.compos_dataframe['height'] > 0.85) &
                                           (self.compos_dataframe['width']/self.compos_dataframe['height'] < 1.15) &
                                           (self.compos_dataframe['center_row'] > self.img.shape[0] * 0.15) &
                                           (self.compos_dataframe['area']/(self.img.shape[0] * self.img.shape[1]) < 0.005) &
                                           (self.compos_dataframe['text_content'].str.len()<=1)].index
        # 限制：1.是多选框 2.比例像个按钮 3.高度限制 4.面积限制 5.文本限制
        for cri in cr_idx:
            cp = self.compos_dataframe.loc[cri]
            if self.compos_dataframe.loc[cri, 'center_column'] >= self.img.shape[1] / 2:  # 在右半边
                row_cand_id = self.compos_dataframe.loc[(cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])  # 中心在图标上下范围内
                                                        & (self.compos_dataframe['center_column'] < cp['center_column']) & (self.compos_dataframe['class'] == 'Text')  # 在图标左边 文本不能为空
                                                        & (self.compos_dataframe['id'] != cri)  # 不能是找回的图标 不能是图标自己
                                                        & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index  # 必须是同一个块
                if len(row_cand_id) > 0:  # 有横向候选
                    selected_idx = abs(self.compos_dataframe.loc[row_cand_id, 'center_column'] - cri).idxmin()
                    if len(self.compos_dataframe.loc[(self.compos_dataframe.loc[selected_idx, 'column_max'] <= self.compos_dataframe['center_column']) & (self.compos_dataframe['center_column'] <= cp['column_min'])
                                                     & (cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])
                                                     & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])]) == 0:
                        # 最近项和当前图标之间不隔着其他东西（cp右边和text左边之间不能有center）+ 限制在同一行看
                        self.compos_dataframe.loc[selected_idx, 'pair_to'] = cri
                        self.compos_dataframe.loc[cri, 'pair_to'] = selected_idx
                        self.compos_dataframe.loc[cri, 'text_content'] = (self.compos_dataframe.loc[selected_idx, 'text_content'] + '的选中按钮') if lang == 'zh' else \
                                                                        ('Selected button of' + self.compos_dataframe.loc[selected_idx, 'text_content'])
            else:  # 在左半边往右边找
                row_cand_id = self.compos_dataframe.loc[(cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])  # 中心在图标上下范围内
                                                        & (self.compos_dataframe['center_column'] > cp['center_column']) & (self.compos_dataframe['class'] == 'Text')  # 在图标左边 文本不能为空
                                                        & (self.compos_dataframe['id'] != cri)  # 不能是找回的图标 不能是图标自己
                ...
                if len(row_cand_id) > 0:  # 有横向候选
                    selected_idx = abs(self.compos_dataframe.loc[row_cand_id, 'center_column'] - cri).idxmin()
                    if len(self.compos_dataframe.loc[(cp['column_max'] <= self.compos_dataframe['center_column']) & (self.compos_dataframe['center_column'] <= self.compos_dataframe.loc[selected_idx, 'column_min'])
                                                     & (cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])
                                                     & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])]) == 0:
                        # 最近项和当前图标之间不隔着其他东西（cp右边和text左边之间不能有center）+ 限制在同一行看
                        self.compos_dataframe.loc[selected_idx, 'pair_to'] = cri
        ...
        # 高的单选多选框可能是识别错了。。。
        # upper_check_box = self.compos_dataframe.loc[(self.compos_dataframe['sub_class'].str.lower().isin(['多选框', '选项'])) &
        #                                             (self.compos_dataframe['center_row'] < self.img.shape[0] * 0.125)].index
        # 其他可能是错误识别的/识别成选项卡的
        other_check_idx = list(set(self.compos_dataframe.loc[self.compos_dataframe['sub_class']
                                   .str.lower().isin(['多选框' if lang=='zh' else 'check box', '选项' if lang=='zh' else 'options'])].index) - set(cr_idx))
        self.compos_dataframe.loc[other_check_idx, 'sub_class'] = 'buttonicon'  # 改回去
        # for ucb in upper_check_box:
        #     if self.compos_dataframe.loc[ucb, 'text_content'] == '':
        #         self.compos_dataframe.loc[ucb, 'sub_class'] = 'buttonicon'
        #     else:
        #         self.compos_dataframe.loc[ucb, 'class'] = 'Text'
        #         self.compos_dataframe.loc[ucb, 'sub_class'] = 'Text'


    '''
    ******************************
    **** Pair Text for Nontext ***
    ******************************
    ...
    ******************************
    '''
    def icon_cls_by_clip(self, ori_img, model_cls, preprocess, device, real_labels_dict, labels_for_read, lang='zh'):
        # 三. 先找回文字
        label = clip.tokenize(list(real_labels_dict.values())).to(device)
        # need_cls_df = pd.DataFrame()
        image_pil = Image.fromarray(cv2.cvtColor(ori_img, cv2.COLOR_BGR2RGB))
        ori_df = self.compos_dataframe.copy()  # 更新
        # 未配对图标找回 （改成所有图标的）
        # （仅垂直）or分类 前提：没有配对 条件1：是按钮或者开关 不能有字 条件2：是图不能有字 条件3：单个文字的文本框 条件4：比例特定的
        need_cls_df = ori_df.loc[((  ( ((ori_df['sub_class'] == 'buttonicon') | (ori_df['sub_class'] == 'switch'))
                                        & ((ori_df['text_content'].str.contains(r'[a-zA-Z0-9]')) | (ori_df['text_content'].str.len() <= 1) ) )
                                  ...
                                  & (ori_df['text_content'] != 'i'))
                                 | (((ori_df['sub_class'] == 'buttonicon') | (ori_df['sub_class'] == 'switch')) &
                                    (ori_df['width']/ori_df['height'] > 1.5) & (ori_df['area']/(self.img.shape[0] * self.img.shape[1]) < 0.2) & (ori_df['text_content']==''))]  # 1273 (ori_df['text_content'].str.len() <= 1)
        need_cls_df = need_cls_df.sort_values(['center_row'])  # 先按上下顺序排 i = np.where(need_cls_df.index.values==15)[0][0] 找
        # 开始找回
        for i in range(len(need_cls_df)):
            cp = need_cls_df.iloc[i]
            curr_id = cp['id']
            # 如果这个元素还在
            if curr_id in self.compos_dataframe.index:
                # 先默认两种找回模式：1.横向向左同一排（取消） 2.垂直居中向下 (都是中心阈值)
                curr_row, curr_col = cp['center_row'], cp['center_column']

                # # # 截图 调试用
                # x_ratio, y_ratio = self.img.shape[1] / ori_img.shape[1], self.img.shape[0] / ori_img.shape[0]
                # x1, x2, y1, y2 = cp['column_min']/x_ratio, cp['column_max']/x_ratio, cp['row_min']/y_ratio, cp['row_max']/y_ratio
                ...
                find_the_text = False
                # 纵向匹配找回
                if 0.8 < cp['width']/cp['height'] < 3 and cp['area'] / (self.img.size / 3) < 0.015:
                    col_cand_id = self.compos_dataframe.loc[(self.compos_dataframe['center_column'] < curr_col + 4) & (curr_col - 15 < self.compos_dataframe['center_column'])  # 限制中线对齐 因为可能有脚标 右边偏移阈值加大
                                                            & (self.compos_dataframe['center_row'] >= cp['center_row']) & (self.compos_dataframe['text_content'] != '')  # 必须上面如下面字 必须有字
                                                            & ~(self.compos_dataframe['text_content'].str.endswith('_'))& (self.compos_dataframe['id'] != curr_id)  # 不能是找回的图标 不能是图标自己
                                                            & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index  # 必须是同一个块
                    if len(col_cand_id) > 0:  # 有竖向候选
                        # col_cand_gap = self.compos_dataframe.loc[col_cand_id, 'center_row'] - curr_row
                        col_cand_gap = self.compos_dataframe.loc[col_cand_id, 'row_min'] - cp['row_max']
                        gap_th = max(20, cp['height']/3)  # 间距阈值
                        if min(col_cand_gap) < gap_th:
                            selected_idx = abs(self.compos_dataframe.loc[col_cand_id, 'center_row'] - curr_row).idxmin()
                            self.compos_dataframe.loc[curr_id, 'text_content'] = self.compos_dataframe.loc[selected_idx, 'text_content'] + '_'  # 成功匹配
                            self.compos_dataframe.loc[curr_id, 'pair_to'] = selected_idx
                            # print('纵向droped', selected_idx)
                            self.compos_dataframe.drop(selected_idx, inplace=True)  # 把找到的配字扔掉 只保留按钮图标和释义
                            find_the_text = True

                # 横向匹配找回(左图右字版)
                '''
                1. 图标本身要求：中心超过页面上15% + 位置在页面左侧（中心小于页面左50%）
                2. 文字要求：1）中心在图标上下范围内 2）在图标右边 文本不能为空 3）不能是找回的图标 不能是图标自己 4）必须是同一个块
                3. 如果有候选 且最近项和当前图标之间不隔着其他东西（cp右边和text左边之间不能有center）
                4. 保留文字 删掉图标
                and cp['center_column'] < (min(cp_block['column_min']) + max(cp_block['column_max'])) * 0.5 必需是block里靠左的图标 （这个条件先不要
                '''
                # cp_block = self.compos_dataframe.loc[self.compos_dataframe['line_split_c'] == cp['line_split_c']]
                if not find_the_text and 0.85 < cp['width'] / cp['height'] < 3 \
                        and cp['area'] / (self.img.size / 3) < 0.025 \
                        and cp['center_row'] > self.img.shape[0] * 0.125 and (len(cp['text_content']) <= 1):
                    row_cand_id = self.compos_dataframe.loc[(cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])  # 中心在图标上下范围内
                                                            & (self.compos_dataframe['center_column'] >= cp['center_column']) & (self.compos_dataframe['class'] == 'Text')  # 在图标右边 文本不能为空
                                                            & (self.compos_dataframe['id'] != curr_id)  # 不能是找回的图标 不能是图标自己
                                                            & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index  # 必须是同一个块
                    if len(row_cand_id) > 0:  # 有横向候选
                        selected_idx = abs(self.compos_dataframe.loc[row_cand_id, 'center_column'] - curr_col).idxmin()
                        if len(self.compos_dataframe.loc[(cp['column_max'] <= self.compos_dataframe['center_column']) &
                                                         (self.compos_dataframe['center_column'] <= self.compos_dataframe.loc[selected_idx, 'column_min'])
                                                         & (cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])
                                                         & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])]) == 0:
                            # 最近项和当前图标之间不隔着其他东西（cp右边和text左边之间不能有center）+ 限制在同一行看
                            self.compos_dataframe.loc[selected_idx, 'pair_to'] = curr_id
                            w, h, img_w, img_h = cp['width'], cp['height'], self.img.shape[1], self.img.shape[0]
                            # 按照比例 判断头像
                            if 0.975 <= w / h <= 1.025 and cp['area'] / (img_w * img_h) > 0.012 and cp['center_column'] < img_w / 3 and cp['center_row'] < img_h / 3:
                                self.compos_dataframe.loc[curr_id, 'text_content'] = (self.compos_dataframe.loc[selected_idx, 'text_content'] + '的头像') if lang == 'zh' else \
                                                                                        ('Avatar of ' + self.compos_dataframe.loc[selected_idx, 'text_content'])
                                self.compos_dataframe.loc[curr_id, 'pair_to'] = selected_idx
                            else:
                                # print('横向droped', curr_id)
                                self.compos_dataframe.drop(curr_id, inplace=True)  # 把图标扔掉
                            find_the_text = True

                if not find_the_text:  # 如果无法找回 才启用图标识别
                    ...
                    if cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.05:  # 面积不能太大
                        with torch.no_grad():  # clip分类预测
                            ...
                            predicted_class_id = np.argmax(probs) + 1  # 173~176是switch (1~5 + 7~8 + 11 + 18~21 + 45)
                            predicted_class = labels_for_read[str(predicted_class_id)]
                            if np.argmax(probs) + 1 == 148 and w/h >= 4:  # 错误补丁
                                predicted_class = '搜索框' if lang == 'zh' else 'Search box'
                            if np.argmax(probs) + 1 in [173, 174] and cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.0005:  # 错误补丁
                                predicted_class = '页面指示器' if lang == 'zh' else 'page indicator'

                            # 预测类别的文字
                            if self.compos_dataframe.loc[self.compos_dataframe['id'] == need_cls_df.iloc[i, 0], 'text_content'].values[0] == '':
                                # 0 比例奇怪 是图片
                                if not 0.5 <= w/h <= 20:
                                    self.compos_dataframe.loc[curr_id, 'text_content'] = '图片_' if lang == 'zh' else 'picture_'
                                # print('id' + str(need_cls_df.iloc[i, 0]), 'area', area_wh)
                                # 1 置信度小的
                                elif (sorted(probs)[-2] + sorted(probs)[-1]) <= 0.4:
                                    # 1.1 图片面积小/比例不奇怪 可能是图标
                                    if cp['area'] / (self.img.shape[0] * self.img.shape[1]) < 0.0015:
                                        self.compos_dataframe.loc[curr_id, 'text_content'] = '可能是' if lang == 'zh' else 'Maybe is a ' + predicted_class + '_'  # + str(max(probs))+ '_'  # 找回注释失败 使用原分类结果
                                        if sorted(probs)[-2] + sorted(probs)[-1] < 0.3:  # 如果置信度太低
                                            self.compos_dataframe.loc[curr_id, 'text_content'] = '图片_' if lang == 'zh' else 'picture_'
                                    # 1.2 图片面积大 可能是头像/图片
                                    else:
                                        self.compos_dataframe.loc[curr_id, 'text_content'] = ('图片，或' if lang == 'zh' else 'a picture or ') + predicted_class + '_'  # 填入df
                                        if 0.975 <= w/h <= 1.025:  # 按照比例 判断头像
                                            ...
                                            if cp['center_column'] < img_w/3 and cp['center_row'] < img_h/3 or cp['center_column'] > img_w*2/3 and cp['center_row'] < img_h*1/3 or img_w/2 - 5 < cp['center_column'] < img_w/2 + 5 and cp['center_row'] < img_h*1/3:
                                                ...
                                                self.compos_dataframe.loc[curr_id, 'text_content'] = ('头像，或' if lang == 'zh' else 'Avatar or ') + predicted_class + '_'  # + str(max(probs)) + '_' + str(area_wh) + '_'  + str(w/h) + '_'
                                # 2 置信度大 是见过的图标
                                else:
                                    if predicted_class == '搜索框' or predicted_class == 'Search box':
                                        if cp['area']/(self.img.shape[0] * self.img.shape[1]) < 0.01:  # 搜索框面积太小的也不对
                                            self.compos_dataframe.loc[curr_id, 'text_content'] = ('图片' if lang == 'zh' else 'Picture') +'_'
                                        else:
                                            self.compos_dataframe.loc[curr_id, 'text_content'] = predicted_class + '_'
                                            self.compos_dataframe.loc[curr_id, 'sub_class'] = "edittext"
                                    else:
                                        if np.argmax(probs) + 1 in [173, 174] and predicted_class != ('页面指示器' if lang == 'zh' else 'page indicator'):
                                            self.compos_dataframe.loc[curr_id, 'sub_class'] = "switch"
                                        # 查看图标是否可以加文字解释：
                                        if cp['center_row'] > self.img.shape[0] * 0.15 and cp['center_column'] > self.img.shape[1] * 0.5:  # 这次是非顶部的 在图片右侧的图标
                                            row_cand_id = self.compos_dataframe.loc[(cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])  # 中心在图标上下范围内
                                                                                    & (self.compos_dataframe['center_column'] < cp['center_column']) & (self.compos_dataframe['class'] == 'Text')  # 在图标左边 必须是文本
                                                                                    & (self.compos_dataframe['id'] != curr_id)  # 不能是找回的图标 不能是图标自己
                                                                                    & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])].index  # 必须是同一个块
                                            if len(row_cand_id) > 0:  # 有横向候选
                                                selected_idx = abs(self.compos_dataframe.loc[row_cand_id, 'center_row']).idxmin()  # 选靠上的
                                                # if len(self.compos_dataframe.loc[(self.compos_dataframe.loc[selected_idx, 'column_max'] <= self.compos_dataframe['center_column']) & (self.compos_dataframe['center_column'] <= cp['column_min'])
                                                #                                  & (cp['row_min'] < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < cp['row_max'])
                                                #                                  & (self.compos_dataframe['line_split_c'] == cp['line_split_c'])]) == 0:
                                                if predicted_class_id == 2:  # 有配对的前进跳过 无意义
                                                    self.compos_dataframe.drop(index=curr_id, inplace=True)
                                                else:
                                                    self.compos_dataframe.loc[curr_id, 'pair_to'] = selected_idx  # 把图标和文字匹配上
                                                    self.compos_dataframe.loc[curr_id, 'text_content'] = (self.compos_dataframe.loc[selected_idx, 'text_content'] + '的' + predicted_class) \
                                                                                                        if lang == 'zh' else ('the ' + predicted_class + ' of ' + self.compos_dataframe.loc[selected_idx, 'text_content'])

                                                    if 173 <= predicted_class_id <= 176:  # 如果是Switch就扔掉原来的文字
                                                        self.compos_dataframe.drop(index=selected_idx, inplace=True)
                                        if (curr_id in self.compos_dataframe.index) and (self.compos_dataframe.loc[curr_id, 'pair_to'] == -1):  # 没有被删除的 且没有加过匹配文字的
                                            # self.compos_dataframe.loc[self.compos_dataframe['id'] == need_cls_df.iloc[i, 0], 'text_content'] += predicted_class + '键' + '_'
                                            self.compos_dataframe.loc[curr_id, 'text_content'] += predicted_class + ('键' if lang == 'zh' else ' button') + '_'  # + str(max(probs)) + '_' # 填入df
    ...
    *****************************
    ...
    def get_back_edittext(self, lang='zh'):  # 五、找回编辑框（输入框）
        # 一、对单独一个Q的文字改回搜索hhh
        ...
        self.compos_dataframe.loc[self.compos_dataframe['text_content'] == 'Q', 'text_content'] = '图标：搜索、放大_' if lang == 'zh' else 'Icon: Search, zoom in_'

        # 二、 搜索框找回
        # 1. 根据搜索
        search_found = False
        search_icon = self.compos_dataframe.loc[self.compos_dataframe['text_content'].str.contains('搜索、放大' if lang == 'zh' else 'Search, zoom in')]
        for search_idx in range(len(search_icon)):
            ...
            if len(row_cand_id) > 0:  # 有横向候选，选列最近的 + 根据搜索框定左右
                ...
                if len(row_cand_gap) > 0:
                    get_id = row_cand_gap.loc[row_cand_gap == sorted(abs(row_cand_gap))[0]].index
                    if len(self.compos_dataframe.loc[get_id, 'center_row'].values) > 0 and self.compos_dataframe.loc[get_id, 'center_row'].values[0] < self.img.shape[0] / 3:
                        self.compos_dataframe.loc[get_id, 'sub_class'] = 'edittext'  # 选择最近的一个长条改为文本框
                        self.compos_dataframe.loc[get_id, 'text_content'] = ('（此处搜索）' if lang == 'zh' else '(Search Here)') + self.compos_dataframe.loc[get_id, 'text_content']  # 加上引导
                        self.compos_dataframe.drop(index=search_icon.iloc[search_idx]['id'], inplace=True)  # 删掉搜索放大键 避免误导
                    search_found = True
        # 2. 根据图片搜索
        if not search_found:
            search_icon = self.compos_dataframe.loc[(self.compos_dataframe['text_content'].str.contains(('相机、图片搜索、或截图、快门' if lang == 'zh' else 'Camera, Image Search, or Screenshot, Shutter'))) |
                                                    (self.compos_dataframe['text_content'].str.contains(('（前后置）摄像头切换' if lang == 'zh' else 'camera switch')))]
            for search_idx in range(len(search_icon)):
                curr_row, curr_col = search_icon.iloc[search_idx]['center_row'], search_icon.iloc[search_idx]['center_column']  # 搜索框中心位置
                # 选取同行 且长宽比>4的
                row_cand_id = self.compos_dataframe.loc[(((curr_row - 5 < self.compos_dataframe['center_row']) & (self.compos_dataframe['center_row'] < curr_row + 5)) |
                ...
                if len(row_cand_id) > 0:  # 有横向候选，选列最近的 + 根据搜索框定左右
                    ...
                    row_cand_gap = row_cand_gap.loc[row_cand_gap < 0] if curr_col > self.img.shape[1] / 2 else row_cand_gap.loc[row_cand_gap > 0]  # 根据搜索图标位置确定匹配方向
                    if len(row_cand_gap) > 0:
                        get_id = row_cand_gap.loc[abs(row_cand_gap) == sorted(abs(row_cand_gap))[0]].index
                        if self.compos_dataframe.loc[get_id, 'center_row'].values[0] < self.img.shape[0] / 3:
                            self.compos_dataframe.loc[get_id, 'sub_class'] = 'edittext'  # 选择最近的一个长条改为文本框
                        search_found = True
        # 3. 根据"搜索"
        if not search_found:
                ...
                if len(row_cand_id) > 0:  # 有横向候选，选列最近的 + 根据搜索框定左右
                    ...
                    if len(row_cand_gap) > 0:
                        ...
                        if self.compos_dataframe.loc[get_id, 'center_row'].values[0] < self.img.shape[0] / 3:
                            self.compos_dataframe.loc[get_id, 'sub_class'] = 'edittext'  # 选择最近的一个长条改为文本框
                            self.compos_dataframe.loc[get_id, 'text_content'] = ('（此处搜索）' if lang == 'zh' else '(Search Here)') + self.compos_dataframe.loc[get_id, 'text_content']  # 加上引导
                            # self.compos_dataframe.drop(index=search_icon.iloc[search_idx]['id'], inplace=True)  # 删掉搜索search等字样 避免误导

        # 三. 适用于地图的加强edit_text找回
        ...
        self.compos_dataframe.loc[(self.compos_dataframe['sub_class'] == 'edittext') | (self.compos_dataframe['sub_class'] == 'autocompletetextview'), 'text_content'] = \
            ('默认文本: ' if lang == 'zh' else 'Default text: ') + self.compos_dataframe.loc[(self.compos_dataframe['sub_class'] == 'edittext') | (self.compos_dataframe['sub_class'] == 'autocompletetextview'), 'text_content']  # '在此处输入'  # ['在此输入'+str(edt_id) for edt_id in list(range(len(self.compos_dataframe.loc[empty_edittext_idx])))]
    ...
    *****************************
    '''
    def find_caption(self, lang='zh'):
        for lsc in self.compos_dataframe['line_split_c'].unique():
            curr_lsc = self.compos_dataframe.loc[self.compos_dataframe['line_split_c'] == lsc].copy()
            curr_lsc['rowncol'] = curr_lsc['center_row'] + curr_lsc['column_min']
            first_text_id = curr_lsc.loc[(curr_lsc['text_content'] != '搜索') & (curr_lsc['text_content'] != '') &
                                         ~(curr_lsc['text_content'].str.endswith('_')) & ~(curr_lsc['sub_class'] == 'edittext') & ~(curr_lsc['sub_class'] == 'image') &
                                         (curr_lsc['pair_to'] == -1)].sort_values('rowncol').index
            # print(curr_lsc['text_content'])

            if len(first_text_id) > 0:
                first_text_id = first_text_id[0]
                # print(self.compos_dataframe.loc[first_text_id, 'text_content'])
                if (curr_lsc.loc[first_text_id, 'center_column'] < self.img.shape[1] * 2 / 3) and \
                        (curr_lsc.loc[first_text_id, 'center_row'] < self.img.shape[0] * 3 / 4) and \
                        len(self.compos_dataframe.loc[(self.compos_dataframe['center_row'] < curr_lsc.loc[first_text_id, 'row_max'])
                                                      & (self.compos_dataframe['center_row'] > curr_lsc.loc[first_text_id, 'row_min'])
                                                      & (self.compos_dataframe['line_split_c'] == curr_lsc.loc[first_text_id, 'line_split_c'])
                                                      & (self.compos_dataframe['id'] != first_text_id)
                                                      & ((self.compos_dataframe['text_content'] != '') & (curr_lsc.loc[first_text_id, 'height'] * 0.9 < self.compos_dataframe['height']) & (self.compos_dataframe['height'] < curr_lsc.loc[first_text_id, 'height'] * 1.1))
                                                      & ~(self.compos_dataframe['text_content'].str.endswith('_'))]) == 0:  # 位置不能太靠右 同行同block不能有别的大小相似文字
                    self.compos_dataframe.loc[first_text_id, 'text_content'] = ('标题：' if lang == 'zh' else 'Caption: ') + self.compos_dataframe.loc[first_text_id, 'text_content']
    ...
    *****************************
    '''
    def find_selected_tab(self, ori_img, lang='zh'):  # 选项卡选中项发现 同一线框里有3~6个均匀分布的同行（center_row差距10以内）时触发
        x_ratio, y_ratio = self.img.shape[1] / ori_img.shape[1], self.img.shape[0] / ori_img.shape[0]  # 截图比例 这里使用原图比对
        # 根据线框（line_split_c）找到疑似选项卡
        lsc_groups = self.compos_dataframe.groupby('line_split_c').groups
        for lsc in lsc_groups:
            lsc_group = self.compos_dataframe.loc[lsc_groups[lsc]]  # 取出一块
            more_class = lsc_group['class'].value_counts().index[0]  # 去噪 选取多数类
            lsc_group = lsc_group.loc[lsc_group['class'] == more_class].sort_values('center_column')  # 顺便加一个排序
            # 1. 选项卡数量限制 2.同行限制 3.高度大小限制
            # print(lsc_group['text_content'])  # 测试用
            if 3 <= len(lsc_group) <= 8 and (max(lsc_group['center_row'])-min(lsc_group['center_row'])) < 70 \
                    and not (max(lsc_group['height'])/min(lsc_group['height']) < 1.8 and len(lsc_group) < 4) \
                    and (sum(lsc_group['center_row'])/len(lsc_group['center_row']) < self.img.shape[0]/4 or
                         sum(lsc_group['center_row'])/len(lsc_group['center_row']) > self.img.shape[0] * 3 / 4):
                h_gaps = lsc_group['column_min'].values[1:] - lsc_group['column_max'].values[:-1]
                if max(h_gaps)-min(h_gaps) <= max(lsc_group['width']) * 0.6:  # 3.分布均匀
                    # size_dif_flag = (max(pd.concat([lsc_group['height'][:2], lsc_group['height'][3:]]))/min(pd.concat([lsc_group['height'][:2], lsc_group['height'][3:]])) < 1.25 and len(lsc_group) == 5) or \
                    #        (max(lsc_group['height'])/min(lsc_group['height']) < 1.25 and len(lsc_group) != 5)
                    size_dif_flag = False
                    # 是选项卡 开始识别！
                    # print(lsc_group['text_content'])  # 测试用
                    hist_all = []  # 所有选项的颜色分布直方图
                    for cp_id in lsc_group.index:
                        x1, x2, y1, y2 = lsc_group.loc[cp_id, ['column_min', 'column_max', 'row_min', 'row_max']]
                        x1, x2, y1, y2 = int(x1/x_ratio), int(x2/x_ratio), int(y1/y_ratio), int(y2/y_ratio)
                        cp_image = ori_img[y1:y2, x1:x2]  # 截图

                        # import matplotlib.pyplot as plt  # 测试用画图
                        # plt.imshow(cp_image[:, :, [2, 1, 0]])
                        # plt.axis('off')  # 关闭坐标轴
                        # plt.show()

                        cp_image_flatten = cp_image.reshape(-1, cp_image.shape[2])  # 一个通道一列 w*h行像素颜色 3列
                        unique_color, counts = np.unique(cp_image_flatten, axis=0, return_counts=True)

                        if size_dif_flag:  # 大小差异不大的时候才会算整张图片
                            cp_image_hsv = cv2.cvtColor(cp_image, cv2.COLOR_BGR2HSV)  # 转颜色空间
                            hist_all.append(cv2.calcHist([cp_image_hsv], [0, 1, 2], None, [256, 256, 256], [0, 256, 0, 256, 0, 256]))
                        else:
                            cp_image_rep = np.zeros((7, 1, 3))  # 新建画布 仅保留五个不同的频率最高的颜色
                            for nc, cid in enumerate(sorted(counts, reverse=True)[:cp_image_rep.shape[0]]):
                                cp_image_rep[nc, 0] = unique_color[np.where(counts == cid)][0]
                            cp_image_rep = cp_image_rep.astype(np.uint8)
                            cp_image_hsv = cv2.cvtColor(cp_image_rep, cv2.COLOR_BGR2HSV)  # 转颜色空间
                            hist_all.append(cv2.calcHist([cp_image_hsv], [0, 1, 2], None, [256, 256, 256], [0, 256, 0, 256, 0, 256]))

                    # 得到颜色相似度矩阵
                    num_images = len(hist_all)
                    similarity_matrix = np.zeros((num_images, num_images))  # 初始化相似度矩阵
                    # 计算相似度矩阵
                    for i in range(num_images):
                        for j in range(i, num_images):
                            hist1 = hist_all[i]
                            hist2 = hist_all[j]
                            # 计算直方图相似度，使用 cv2.HISTCMP_CORREL（相关性）作为相似度度量
                            similarity = cv2.compareHist(hist1, hist2, cv2.HISTCMP_CORREL)  # 相似度计算 cv2.HISTCMP_CORREL 相关性  cv2.HISTCMP_CHISQR 卡方  cv2.HISTCMP_BHATTACHARYYA 巴氏距离
                            # 将相似度放入矩阵的对应位置
                            similarity_matrix[i, j] = similarity
                            similarity_matrix[j, i] = similarity  # 因为是对称矩阵，所以可以复制一份到对角线上
                    # 计算每行的相似度之和
                    row_sums = similarity_matrix.sum(axis=1)
                    # print(row_sums)
                    # row_sums = row_sums/max(row_sums)
                    # print(row_sums, '\n')
                    if num_images == 5 and sum(lsc_group['center_row'])/len(lsc_group) > self.img.shape[0] * 3/4:  # 一行五个的特殊情况 先丢掉正中间 如果有满足的就判断它是选中
                        without_center = np.delete(np.delete(similarity_matrix, 2, axis=0), 2, axis=1).sum(axis=1)  # 这种情况下要先丢中间一行一列再相加
                        if max(without_center)-min(without_center) >= 0.2 and self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] != '' and self.compos_dataframe.loc[lsc_group.index[np.argmin(without_center)], 'text_content']!='图片_':
                            select_one = np.argmin(without_center) + 1 if np.argmin(without_center) >= 2 else np.argmin(without_center)
                            self.compos_dataframe.loc[lsc_group.index[select_one], 'text_content'] += '（已选中）' if lang == 'zh' else '(Selected)'  # 找到了选中项
                            # print('选中项：', self.compos_dataframe.loc[lsc_group.index[np.argmin(without_center)], 'text_content'])
                        elif max(row_sums)-min(row_sums) >= 0.5 and self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] != '' and self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] != '图片_':  # 放松条件 允许加入第三个
                            self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] += '（已选中）' if lang == 'zh' else '(Selected)'  # 找到了选中项
                            # print('选中项：', self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'])
                    elif max(row_sums)-min(row_sums) >= 0.1 and self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] != '' and self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] != '图片_':  # 其他数量直接判断
                            self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'] += '（已选中）' if lang == 'zh' else '(Selected)'  # 找到了选中项
                            # print('选中项：', self.compos_dataframe.loc[lsc_group.index[np.argmin(row_sums)], 'text_content'])
    ...
    *****************************
    '''
    def switch_name(self, lang='zh'):
        switch_idx = self.compos_dataframe.loc[(self.compos_dataframe['sub_class'] == 'switch') & (self.compos_dataframe['pair_to'] == -1)].index
        for sidx in switch_idx:
            if sidx in self.compos_dataframe.index:
                if self.compos_dataframe.loc[sidx, 'pair_to'] != -1:
                    paired_text_idx = [self.compos_dataframe.loc[sidx, 'pair_to']]
                else:
                    paired_text_idx = self.compos_dataframe.loc[(self.compos_dataframe['line_split_c'] == self.compos_dataframe.loc[sidx, 'line_split_c']) & (self.compos_dataframe['id'] != sidx) &
                                                                (self.compos_dataframe['text_content'] != '') & (abs(self.compos_dataframe['center_row']-self.compos_dataframe.loc[sidx, 'center_row']) < 15) &
                                                                (self.compos_dataframe['center_column'] < self.compos_dataframe.loc[sidx, 'center_column'])].index
                if len(paired_text_idx) != 0:
                    throw_pid = []
                    for n, pid in enumerate(paired_text_idx):
                        if pid not in self.compos_dataframe.index:  # 有的pair_to被合并了
                            paired_ori = self.compos_dataframe.loc[(self.compos_dataframe['pair_to']==pid) & (self.compos_dataframe['text_content'] != '') & (self.compos_dataframe['id']!=sidx)].index
                            if len(paired_ori) > 0:  # 还原
                                paired_text_idx[n] = paired_ori[0]
                            else:
                                throw_pid.append(pid)
                    paired_text_idx = list(set(paired_text_idx)-set(throw_pid))
                    if len(paired_text_idx) != 0:
                        if lang == 'zh':
                            self.compos_dataframe.loc[sidx, 'text_content'] = \
                                ''.join(self.compos_dataframe.loc[paired_text_idx, 'text_content']).replace('标题：' if lang == 'zh' else 'Caption: ', '') \
                                + '的' + self.compos_dataframe.loc[sidx, 'text_content']
                        else:
                            self.compos_dataframe.loc[sidx, 'text_content'] = 'the ' + self.compos_dataframe.loc[sidx, 'text_content'] + ' of ' + \
                                                                              ''.join(self.compos_dataframe.loc[paired_text_idx, 'text_content']).replace('标题：' if lang == 'zh' else 'Caption: ', '')
                        self.compos_dataframe = self.compos_dataframe.drop(index=paired_text_idx)
    ...
    *****************************
    '''
    def other_sub_class_text(self, lang='zh'):
        # 九、这里加一个除了buttonicon和text 其他类的、没有文本内容的专属名称
        # other_df = self.compos_dataframe.loc[~(self.compos_dataframe['sub_class'].str.lower().isin
        #                                      (['buttonicon', 'text', 'edittext', 'switch', '多选框', '选项'])) &
        #                                      (self.compos_dataframe['pair_to'] == -1)]
        other_df = self.compos_dataframe.loc[self.compos_dataframe['text_content'] == '']
        for one_df_idx in other_df.index:
            ori_text = self.compos_dataframe.loc[one_df_idx, 'text_content']
            if ori_text == '':
                self.compos_dataframe.loc[one_df_idx, 'text_content'] = ('一个' if lang == 'zh' else 'a ') + self.compos_dataframe.loc[one_df_idx, 'sub_class']
            else:
                self.compos_dataframe.loc[one_df_idx, 'text_content'] = self.compos_dataframe.loc[one_df_idx, 'sub_class'] + ': ' + ori_text
