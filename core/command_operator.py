...
def perform_action(action_data):
    ...
    if action_type == 'swipe':
        ...
        adb_command_enable_adb_ime = f"adb -s {DeviceName} shell ime enable {ADBKeyboard}"
        adb_command_change2adbkeyboard = f"adb -s {DeviceName} shell ime set {ADBKeyboard}"
...
def capture_screenshot(screenshot_path):
    ...
    try:
        ...
        print(f"fail to get the screenshot! : {str(e)}")
