...
save_path_old = 'data/screenshot/screenshot_calc_old.png'
save_path_new = 'data/screenshot/screenshot_calc_new.png'
save_path_default = 'data/screenshot/screenshot_calc_default.png'
...
# 1. Get the list of valid input methods for the current device
# adb shell ime list -s
# Note: -s is not all installed input methods, but the input methods that are installed and checked.
# Enable method:
# System Settings >> General >> Language & Input >> Check the input method

# 2. Check all available virtual keyboards:
# adb shell ime list -a

# 3. Get detailed information about valid input methods for the current device
# adb -s ALNYUN3829G02254 shell ime list

# 4. View the currently used input method
# When we encapsulate the switch, we must first know the default name of this input method. The following is the way to get it:
