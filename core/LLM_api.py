...
class ChatGptLLM(LLM):
    def get_llm_result(self):
        ...
        payload = {
            "model": "gpt-4-turbo",
            "messages": json.loads(Global_LLm_history.get_payload()).get("messages")
        }
...
class AzureChatGptLLM(LLM):
    def get_llm_result(self):
        ...
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer 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'
        }
        Global_LLm_history.add_user_input(self.user_input)
        
        payload = {
            "model": "gpt-4-turbo",
            "messages": Global_LLm_history.get_payload()
        }
